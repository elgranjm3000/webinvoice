...
export default async function Proveedores({
  ...
  return (
    <>
      <PageHeader
        title="Proveedores"
        subtitle="Quiénes te venden — base del libro de compras"
        action={
          <a href="/proveedores?nuevo=1" className="border border-tinta bg-white px-5 py-2.5 text-[14px] font-medium transition-colors hover:bg-papel-2">
            Registrar proveedor
          </a>
        }
      />
      ...
      <Modal
        onCloseHref="/proveedores"
        open={Boolean(editing) || nuevo === "1"}
        title={editing ? `Editar proveedor: ${editing.legal_name}` : "Registrar proveedor"}
      >
        <form action={editing ? updateSupplier : createSupplier}>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="block text-[13px] text-tinta-suave">
            Número de RIF
            <input name="tax_id" required placeholder="J-30111222-3" defaultValue={editing?.tax_id ?? ""} className={`num mt-1.5 ${inp}`} />
          </label>
          <label className="block text-[13px] text-tinta-suave sm:col-span-2">
            Dirección fiscal
        ...
        <div className="mt-6 flex items-center gap-4">
          ...
          {editing && (
            <a href="/proveedores" className="text-[13px] text-tinta-suave underline-offset-2 hover:underline">
              Cancelar
            </a>
          )}
        </div>
      ...
      {rows.length === 0 ? (
        <EmptyState
          title={term ? `Ningún proveedor coincide con «${term}»` : "Sin proveedores registrados"}
          hint={term ? "Prueba con otra razón social o RIF." : "Registra tu primer proveedor para poder registrar sus facturas de compra."}
        />
      ) : (
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-regla">
                ...
                <td className="py-3 pl-4 text-right">
                  <RowMenu editHref={`/proveedores?edit=${r.id}`}>
